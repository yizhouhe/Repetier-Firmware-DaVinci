...
void Commands::printCurrentPosition() {
    ...
    Com::printFLN(Com::tSpaceEColon,
                  Printer::currentPositionSteps[E_AXIS] * Printer::invAxisStepsPerMM[E_AXIS] * (Printer::unitIsInches ? 0.03937 : 1),
                  4);
#ifdef DEBUG_POS
...
void Commands::printTemperatures(bool showRaw) {
    ...
    Com::printF(Com::tSpaceSlash,
                Extruder::current->tempControl.targetTemperatureC, 0);
#else
    Com::printF(Com::tTColon, temp);
    Com::printF(Com::tSpaceSlash,
                Extruder::current->tempControl.targetTemperatureC, 0);
    ...
    Com::printF(Com::tSpaceBAtColon,
                (pwm_pos[heatedBedController.pwmIndex])); // Show output of auto
                                                          // tune when tuning!
#endif
#endif
    Com::printF(
        Com::tSpaceAtColon,
        (autotuneIndex == 255
             ? pwm_pos[Extruder::current->id]
             : pwm_pos[autotuneIndex])); // Show output of auto tune when tuning!
#if NUM_EXTRUDER > 1 && MIXING_EXTRUDER == 0
    for (uint8_t i = 0; i < NUM_EXTRUDER; i++) {
        Com::printF(Com::tSpaceT, (int)i);
        Com::printF(Com::tColon, extruder[i].tempControl.getStatefulTemperature());
        Com::printF(Com::tSpaceSlash, extruder[i].tempControl.targetTemperatureC,
                    0);
        Com::printF(Com::tSpaceAt, (int)i);
        Com::printF(
            Com::tColon,
            (pwm_pos[extruder[i].tempControl.pwmIndex])); // Show output of auto
                                                          // tune when tuning!
...
void Commands::reportPrinterUsage() {
    ...
    for (uint8_t i = 0; i < NUM_EXTRUDER; i++)
        if (tempController[i]->targetTemperatureC > 15)
            alloff = false;
#endif
    int32_t seconds = (alloff
                           ? 0
                           : (HAL::timeInMilliseconds() - Printer::msecondsPrinting) / 1000)
        + HAL::eprGetInt32(EPR_PRINTING_TIME);
...
void setMotorCurrent(uint8_t channel, unsigned short level) {
    ...
    // NOTE: Do not increase the current endlessly. In case the engine reaches its
    // current saturation, the engine and the driver can heat up and loss power.
    // When the saturation is reached, more current causes more heating and more
    // power loss. In case of engines with lower quality, the saturation current
    // may be reached before the nominal current.
...
#if TMC2130_ON_X
    Com::printF(Com::tSpaceXColon,
                (uint32_t)(Printer::tmc_driver_x->rms_current()));
#endif
#if TMC2130_ON_Y
    Com::printF(Com::tSpaceYColon,
                (uint32_t)(Printer::tmc_driver_y->rms_current()));
#endif
#if TMC2130_ON_Z
    Com::printF(Com::tSpaceZColon,
                (uint32_t)(Printer::tmc_driver_z->rms_current()));
#endif
#if TMC2130_ON_EXT0
    Com::printF(Com::tSpaceEColon,
                (uint32_t)(Printer::tmc_driver_e0->rms_current()));
#endif
#if TMC2130_ON_EXT1
...
void dacAnalogUpdate(bool saveEEPROM = false) {
    ...
    dacSimpleCommand(
        (uint8_t)MCP4728_CMD_GC_UPDATE); // MCP4728 General Command Software
                                         // Update (Update all DAC Outputs to
                                         // reflect settings)

    // if (saveEEPROM) dacReadStatus(); // Not necessary, just a read-back sanity
    // check.
}

void dacCommitEeprom() {
    dacAnalogUpdate(true);
    dacReadStatus(); // Refresh EEPROM Values with values actually stored in
                     // EEPROM. .
}
...
void dacPrintValues() {
    const char* dacChannelPrefixes[] = { Com::tSpaceXColon, Com::tSpaceYColon,
                                         Com::tSpaceZColon, Com::tSpaceEColon };
    ...
    dacPrintSet(dac_motor_current,
                dacChannelPrefixes); // And another for the RUNTIME set
}
...
void motorCurrentControlInit() { // Initialize MCP4728 Motor Current
    ...
    dacSimpleCommand(
        (uint8_t)MCP4728_CMD_GC_RESET); // MCP4728 General Command Reset
    dacReadStatus();                    // Load Values from EEPROM.

    for (int i = 0; i < MCP4728_NUM_CHANNELS; i++) {
        setMotorCurrent(
            dac_stepper_channel[i],
            _valuesEp[i]); // This is not strictly necessary, but serves as a good
                           // sanity check to ensure we're all on the same page.
    }
}
#endif
...
#if TMC2130_ON_X
    Com::printF(Com::tSpaceXColon,
                (uint32_t)(Printer::tmc_driver_x->microsteps()));
#endif
#if TMC2130_ON_Y
    Com::printF(Com::tSpaceYColon,
                (uint32_t)(Printer::tmc_driver_y->microsteps()));
#endif
#if TMC2130_ON_Z
    Com::printF(Com::tSpaceZColon,
                (uint32_t)(Printer::tmc_driver_z->microsteps()));
#endif
#if TMC2130_ON_EXT0
    Com::printF(Com::tSpaceEColon,
                (uint32_t)(Printer::tmc_driver_e0->microsteps()));
#endif
#if TMC2130_ON_EXT1
...
*/
    ...
    if (!Printer::setDestinationStepsFromGCode(com))
        return; // For X Y Z E F
    float offset[2] = { Printer::convertToMM(com->hasI() ? com->I : 0),
                        Printer::convertToMM(com->hasJ() ? com->J : 0) };
    float target[E_AXIS_ARRAY] = {
        Printer::realXPosition(), Printer::realYPosition(),
        Printer::realZPosition(),
        Printer::destinationSteps[E_AXIS] * Printer::invAxisStepsPerMM[E_AXIS]
    };
    float r;
    if (com->hasR()) {
        /*
    We need to calculate the center of the circle that has the designated radius
    and passes through both the current position and the target position. This
    method calculates the following set of equations where [x,y] is the vector
    from current to target position, d == magnitude of that vector, h ==
    hypotenuse of the triangle formed by the radius of the circle, the distance
    to the center of the travel vector. A vector perpendicular to the travel
    vector [-y,x] is scaled to the length of h [-y/d*h, x/d*h] and added to the
    center of the travel vector [x/2,y/2] to form the new point [i,j] at
    [x/2-y/d*h, y/2+x/d*h] which will be the center of our arc.
    ...
    */
        ...
        // If r is smaller than d, the arc is now traversing the complex plane
        // beyond the reach of any real CNC, and thus - for practical reasons - we
        // will terminate promptly:
        if (isnan(h_x2_div_d)) {
            Com::printErrorFLN(Com::tInvalidArc);
            return;
        }
        // Invert the sign of h_x2_div_d if the circle is counter clockwise (see
        // sketch below)
        if (com->G == 3) {
    ...
    When offset is positive, the left hand circle will be generated - when it is
    negative the right hand circle is generated.


    T  <-- Target position

    ^
    Clockwise circles with this center         |          Clockwise circles with
    this center will have will have > 180 deg of angular travel      | < 180 deg
    of angular travel, which is a good thing! \         |          / center of
    arc when h_x2_div_d is positive ->  x <----- | -----> x <- center of arc
    when h_x2_div_d is negative
    |
    |

    C  <-- Current position                                 */

        // Negative R is g-code-alias for "I want a circle with more than 180
        // degrees of travel" (go figure!), even though it is advised against ever
        // generating such circles in a single line of g-code. By inverting the sign
        // of h_x2_div_d the center of the circles is placed on the opposite side of
        // the line of travel and thus we get the inadvisable long arcs as
        // prescribed.
        if (r < 0) {
            h_x2_div_d = -h_x2_div_d;
...
void Commands::processGCode(GCode* com) {
    ...
    case 10: // G10 S<1 = long retract, 0 = short retract = default> retracts
             // filament according to stored setting
#if NUM_EXTRUDER > 1
...
#endif
        break;
    case 11: // G11 S<1 = long retract, 0 = short retract = default> = Undo
             // retraction according to stored setting
#if NUM_EXTRUDER > 1
    ...
    case 100: { // G100 Calibrate floor or rod radius
        ...
        // G100 [X] [Y] [Z] set floor for argument passed in. Number ignored and may
        // be absent. G100 R with X Y or Z flag error, sets only floor or radius,
        // not both. G100 R[n] Add n to radius. Adjust to be above floor if
        // necessary G100 R[0] set radius based on current z measurement. Moves to
        // (0,0,0)
        float currentZmm = Printer::currentPosition[Z_AXIS];
        if (currentZmm / Printer::zLength > 0.1) {
            Com::printErrorFLN(
                PSTR("Calibration code is limited to bottom 10% of Z height"));
            break;
        }
        if (com->hasR()) {
            if (com->hasX() || com->hasY() || com->hasZ())
                ...
                if (Printer::currentPosition[X_AXIS] == 0 && Printer::currentPosition[Y_AXIS] == 0) {
                    if (Printer::isLargeMachine()) {
                        // calculate radius assuming we are at surface
                        // If Z is greater than 0 it will get calculated out for correct
                        // radius Use either A or B tower as they anchor x Cartesian axis
                        // and always have Radius distance to center in simplest set up.
                    ...
                    Com::printErrorFLN(
                        PSTR("First move to touch at x,y=0,0 to auto-set radius."));
            }
        } else {
            bool tooBig = false;
            if (com->hasX()) {
        ...
        // after adjusting zero, physical position is out of sync with memory
        // position this could cause jerky movement or push head into print surface.
        // moving gets back into safe zero'ed position with respect to newle set
        // floor or Radius.
        Printer::moveTo(IGNORE_COORDINATE, IGNORE_COORDINATE, 12.0,
    ...
    case 131: { // G131 Remove offset
        ...
        Printer::offsetX = 0;
        Printer::offsetY = 0;
        Printer::moveToReal(cx, cy, cz, IGNORE_COORDINATE,
    ...
    case 132: { // G132 Calibrate endstop offsets
        ...
        Printer::setAutolevelActive(
            false); // don't let transformations change result!
        Printer::coordinateOffset[X_AXIS] = 0;
        Printer::coordinateOffset[Y_AXIS] = 0;
        Printer::coordinateOffset[Z_AXIS] = 0;
        // I think this is coded incorrectly, as it depends on the start position of
        // the of the hot end, and so should first move to x,y,z= 0,0,0, but as that
        // may not be possible if the printer is not in the homes/zeroed state, the
        // printer cannot safely move to 0 z coordinate without crashing into the
        // print surface. so other than commenting, I'm not meddling. but you will
        // always get different counts from different positions.
        Printer::deltaMoveToTopEndstops(Printer::homingFeedrate[Z_AXIS]);
        int32_t m = RMath::max(Printer::stepsRemainingAtXHit,
                               RMath::max(Printer::stepsRemainingAtYHit,
                                          Printer::stepsRemainingAtZHit));
    ...
    case 133: { // G133 Measure steps to top
        ...
        // similar to comment above, this will get a different answer from any
        // different starting point so it is unclear how this is helpful. It must
        // start at a well defined point.
        Printer::deltaMoveToTopEndstops(Printer::homingFeedrate[Z_AXIS]);
        ...
        PrintLine::moveRelativeDistanceInSteps(
            0, 0, Printer::axisStepsPerMM[Z_AXIS] * -ENDSTOP_Z_BACK_MOVE, 0,
            Printer::homingFeedrate[Z_AXIS] / ENDSTOP_X_RETEST_REDUCTION_FACTOR,
            true, false);
        Printer::homeAxis(true, true, true);
    } break;
    case 135: // G135
        Com::printF(PSTR("CompDelta:"),
...
#if JSON_OUTPUT
        if (com->hasString() && com->text[1] == '2') { // M20 S2 P/folder or M20 S2 P0:/folder
            if (com->text[3] == 'P') {
            ...
            if (pin_number > -1) {
                if (com->hasS()) {
                    if (com->S >= 0 && com->S <= 255) {
                        ...
                        // analogWrite(pin_number, com->S); // con conflict setting pwm
                        // crashing firmware
                ...
                Com::printErrorFLN(
                    PSTR("Pin can not be set by M42, is in sensitive pins! "));
            }
        }
        break;
...
case 73: // M73 - Print status on display
    ...
    case 92: // M92
        if (com->hasX())
            Printer::axisStepsPerMM[X_AXIS] = com->X;
        if (com->hasY())
            Printer::axisStepsPerMM[Y_AXIS] = com->Y;
        if (com->hasZ())
            Printer::axisStepsPerMM[Z_AXIS] = com->Z;
        Printer::updateDerivedParameter();
        if (com->hasE()) {
    ...
    case 104: // M104 temperature
        ...
        if (com->hasS()) {
            ...
            if (com->hasT() && com->T < NUM_EXTRUDER)
                Extruder::setTemperatureForExtruder(
                    extruder[com->T].tempControl.preheatTemperature + (com->hasO() ? com->O : 0),
                    com->T, com->hasF() && com->F > 0);
            else
                Extruder::setTemperatureForExtruder(
                    Extruder::current->tempControl.preheatTemperature + (com->hasO() ? com->O : 0),
                    Extruder::current->id, com->hasF() && com->F > 0);
        }
#endif
        break;
...
#endif
        break;
    case 105: // M105  get temperature. Always returns the current temperature,
              // doesn't wait until move stopped
    ...
    case 155: // M155 S<1/0> Enable/disable auto report temperatures. When enabled
              // firmware will emit temperatures every second.
...
//end davinci specific
    ...
    case 163: // M163 S<extruderNum> P<weight>  - Set weight for this mixing
        ...
        break;
    case 164: /// M164 S<virtNum> P<0 = dont store eeprom,1 = store to eeprom> -
    ...
    case 200: { // M200 T<extruder> D<diameter>
        ...
        if (d == 0) {
            Com::printFLN(PSTR("Disabled volumetric extrusion for extruder "),
                          static_cast<int>(extruderId));
        } else {
            Com::printF(PSTR("Set volumetric extrusion for extruder "),
                        static_cast<int>(extruderId));
    ...
    case 201: // M201
        if (com->hasX())
            Printer::maxAccelerationMMPerSquareSecond[X_AXIS] = com->X;
        if (com->hasY())
            Printer::maxAccelerationMMPerSquareSecond[Y_AXIS] = com->Y;
        if (com->hasZ())
            Printer::maxAccelerationMMPerSquareSecond[Z_AXIS] = com->Z;
        if (com->hasE())
    ...
    case 202: // M202
        if (com->hasX())
            Printer::maxTravelAccelerationMMPerSquareSecond[X_AXIS] = com->X;
        if (com->hasY())
            Printer::maxTravelAccelerationMMPerSquareSecond[Y_AXIS] = com->Y;
        if (com->hasZ())
            Printer::maxTravelAccelerationMMPerSquareSecond[Z_AXIS] = com->Z;
        if (com->hasE())
    ...
    case 280: // M280
        ...
        if (com->hasS() && com->S > 0) {
#if LAZY_DUAL_X_AXIS
            PrintLine::moveRelativeDistanceInSteps(-Extruder::current->xOffset, 0, 0,
                                                   0, EXTRUDER_SWITCH_XY_SPEED, true,
                                                   true);
#endif
            Extruder::current = &extruder[1];
            PrintLine::moveRelativeDistanceInSteps(
                -Extruder::current->xOffset + static_cast<int32_t>(Printer::xLength * 0.5 * Printer::axisStepsPerMM[X_AXIS]),
                0, 0, 0, EXTRUDER_SWITCH_XY_SPEED, true, true);
        ...
        Printer::updateCurrentPosition(true);
#else
        if (com->hasS()) { // Set ditto mode S: 0 = off, 1 = 1 extra extruder, 2 = 2
            // extra extruder, 3 = 3 extra extruders
...
#if FEATURE_WATCHDOG
    {
        ...
        Com::printInfoFLN(
            PSTR("Triggering watchdog. If activated, the printer will reset."));
        Printer::kill(false);
        HAL::delayMilliseconds(
            200); // write output, make sure heaters are off for safety
#if !defined(__AVR_ATmega1280__) && !defined(__AVR_ATmega2560__)
        InterruptProtectedBlock noInts; // don't disable interrupts on mega2560 and
                                        // mega1280 because of bootloader bug
#endif
        while (1) {
        } // Endless loop
    }
#else
        Com::printInfoFLN(
            PSTR("Watchdog feature was not compiled into this version!"));
#endif
    break;
...
case 300: { // M300
...
#endif
    case 302: // M302 S<0 or 1> - allow cold extrusion. Without S parameter it
              // will allow. S1 will allow, S0 will disallow.
...
#if DISTORTION_CORRECTION
    case 323: // M323 S0/S1 enable disable distortion correction P0 = not
              // permanent, P1 = permanent = default
        if (com->hasS()) {
            if (com->S > 0)
    ...
    case 340: // M340
        ...
        break;
#endif          // FEATURE_SERVO
    case 350: { // M350 Set micro stepping mode. Warning: Steps per unit remains
                // unchanged. S code sets stepping mode for all drivers.
        ...
        if (com->hasP())
            microstepMode(4, (uint8_t)com->P); // Original B but is not supported here
        if (com->hasR())
            microstepMode(5, (uint8_t)com->R);
        microstepReadings();
#endif
    } break;
    ...
    case 402: // M402 Go to stored position
        Printer::GoToMemoryPosition(com->hasX(), com->hasY(), com->hasZ(),
                                    com->hasE(),
    ...
    case 453:
        ...
        break;
#if FAN_THERMO_PIN > -1
    case 460: // M460 X<minTemp> Y<maxTemp> : Set temperature range for thermo
              // controlled fan
        if (com->hasX())
    ...
    case 531:
        ...
        break;
        //- M532 X<percent> L<curLayer> - update current print state progress
        //(X=0..100) and layer L
    case 532:
        if (com->hasX())
  ...
  #if NONLINEAR_SYSTEM
  Com::printF(PSTR("Nonlin. position
  steps:"),Printer::currentDeltaPositionSteps[A_TOWER]);
  ...
  break;*/
    ...
    case 606: {
        ...
        if (Printer::isPositionAllowed(newXpos, Printer::currentPosition[Y_AXIS],
                                       Printer::currentPosition[Z_AXIS])) {
            Printer::moveToReal(newXpos, IGNORE_COORDINATE, IGNORE_COORDINATE,
                                IGNORE_COORDINATE,
                                (com->hasF() ? com->F : EXTRUDER_SWITCH_XY_SPEED));
    ...
    case 907: { // M907 Set digital trimpot/DAC motor current using axis codes.
        ...
        // If "S" is specified, use that as initial default value, then update each
        // axis w/ specific values as found later.
        if (com->hasS()) {
            for (int i = 0; i < 10; i++) {
...
void Commands::executeGCode(GCode* com) {
    ...
    // Set return channel for private commands. By default all commands send to
    // all receivers.
...
//end Da Vinci specific
    ...
    if (com->hasG())
        ...
        if (!Printer::failedMode) {
            // com->printCommand(); // for testing if this the source of extruder
            // switches
            Commands::waitUntilEndOfAllMoves();
