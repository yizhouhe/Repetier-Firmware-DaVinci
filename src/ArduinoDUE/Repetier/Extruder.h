...
#define TEMPERATURE_CONTROLLER_FLAG_DECOUPLE_HOLD                              \
  4                                              ///< Holding target temperature
#define TEMPERATURE_CONTROLLER_FLAG_SENSDEFECT 8 ///< Indicating sensor defect
#define TEMPERATURE_CONTROLLER_FLAG_SENSDECOUPLED                              \
  16                                       ///< Indicating sensor decoupling
#define TEMPERATURE_CONTROLLER_FLAG_JAM 32 ///< Indicates a jammed filament
#define TEMPERATURE_CONTROLLER_FLAG_SLOWDOWN                                   \
  64 ///< Indicates a slowed down extruder
#define TEMPERATURE_CONTROLLER_FLAG_FILAMENTCHANGE                             \
  128 ///< Indicates we are switching filament

#ifndef PID_TEMP_CORRECTION
#define PID_TEMP_CORRECTION 2.0
#endif

/** TemperatureController manages one heater-temperature sensor loop. You can
have up to 4 loops allowing pid/bang bang for up to 3 extruder and the heated
bed.

*/
class TemperatureController {
public:
  uint8_t pwmIndex; ///< pwm index for output control. 0-2 = Extruder, 3 = Fan,
                    ///< 4 = Heated Bed
  uint8_t sensorType; ///< Type of temperature sensor.
  uint8_t sensorPin;  ///< Pin to read extruder temperature.
  int8_t heatManager; ///< How is temperature controlled. 0 = on/off, 1 =
                      ///< PID-Control, 3 = dead time control
  int16_t currentTemperature; ///< Current temperature value read from sensor.
  // int16_t targetTemperature; ///< Target temperature value in units of
  // sensor.
  float currentTemperatureC; ///< Current temperature in degC.
  float targetTemperatureC;  ///< Target temperature in degC.
  float temperatureC;     ///< For 1s updates temperature and last build a short
                          ///< time history
  float lastTemperatureC; ///< Used to compute D errors.
  uint32_t
      lastTemperatureUpdate; ///< Time in millis of the last temperature update.
...
#define deadTime pidPGain
  // deadTime is logically different value but physically overlays pidPGain for
  // saving space
  float pidPGain; ///< Pgain (proportional gain) for PID temperature control
                  ///< [0,01 Units].
  float
      pidIGain; ///< Igain (integral) for PID temperature control [0,01 Units].
...
#endif
  /** Return currentTemperatureC but -333 on defect sensor and -444 on decoupled
   * sensor. */
...
//end davinci specific
  ...
  inline void setDecoupleFull(bool on) {
    flags &= ~(TEMPERATURE_CONTROLLER_FLAG_DECOUPLE_FULL |
               TEMPERATURE_CONTROLLER_FLAG_DECOUPLE_HOLD);
    if (on)
      flags |= TEMPERATURE_CONTROLLER_FLAG_DECOUPLE_FULL;
  }
  ...
  inline void setDecoupleHold(bool on) {
    flags &= ~(TEMPERATURE_CONTROLLER_FLAG_DECOUPLE_FULL |
               TEMPERATURE_CONTROLLER_FLAG_DECOUPLE_HOLD);
    if (on)
      flags |= TEMPERATURE_CONTROLLER_FLAG_DECOUPLE_HOLD;
  }
  ...
  inline void startHoldDecouple(millis_t &t) {
    if (isDecoupleHold())
      return;
    if (fabs(currentTemperatureC - targetTemperatureC) + 1 >
  ...
  inline void stopDecouple() { setDecoupleFull(false); }
...
//end davinici specific
  ...
  inline bool isJammed() { return flags & TEMPERATURE_CONTROLLER_FLAG_JAM; }
  ...
  void autotunePID(float temp, uint8_t controllerId, int maxCycles,
                   bool storeResult, int method);
  inline void startPreheatTime() {
    preheatStartTime = HAL::timeInMilliseconds();
  }
  inline void resetPreheatTime() { preheatStartTime = 0; }
  inline millis_t preheatTime() {
    return preheatStartTime == 0 ? 0
                                 : HAL::timeInMilliseconds() - preheatStartTime;
  }
};
class Extruder;
...
#define _TEST_EXTRUDER_JAM(x, pin)                                             \
  {                                                                            \
    ...
    extruder[x].jamStepsSinceLastSignal += extruder[x].jamLastDir;             \
    if (extruder[x].jamLastSignal != sig &&                                    \
        abs(extruder[x].jamStepsSinceLastSignal -                              \
            extruder[x].jamLastChangeAt) > JAM_MIN_STEPS) {                    \
      if (sig) {                                                               \
        extruder[x].resetJamSteps();                                           \
      }                                                                        \
      extruder[x].jamLastSignal = sig;                                         \
      extruder[x].jamLastChangeAt = extruder[x].jamStepsSinceLastSignal;       \
    } else if (abs(extruder[x].jamStepsSinceLastSignal) >                      \
                   extruder[x].jamErrorSteps &&                                \
               !Printer::isDebugJamOrDisabled() &&                             \
               !extruder[x].tempControl.isJammed() &&                          \
               !extruder[x].tempControl.isFilamentChange()) {                  \
...
#define EXTRUDER_FLAG_WAIT_JAM_STARTCOUNT                                      \
  2 ///< Waiting for the first signal to start counting
...
public:
  ...
  uint8_t enableOn;
  //  uint8_t invertDir; ///< 1 if the direction of the extruder should be
  //  inverted.
  ...
  int16_t watchPeriod;     ///< Time in seconds, a M109 command will wait to
                           ///< stabilize temperature
  int16_t waitRetractTemperature; ///< Temperature to retract the filament when
                                  ///< waiting for heat up
  int16_t waitRetractUnits; ///< Units to retract the filament when waiting for
                            ///< heat up
#if USE_ADVANCE
  ...
  inline void setWaitJamStartcount(bool on) {
    if (on)
      flags |= EXTRUDER_FLAG_WAIT_JAM_STARTCOUNT;
    else
      flags &= ~(EXTRUDER_FLAG_WAIT_JAM_STARTCOUNT);
  }
  ...
  inline bool isLeftCarriage() {
    return ((id == 0 && X_HOME_DIR == -1) || (id == 1 && X_HOME_DIR == 1))
               ? true
               : false;
  }
#endif
...
#define TEMP_INT_TO_FLOAT(temp)                                                \
  ((float)(temp) / (float)(1 << CELSIUS_EXTRA_BITS))
