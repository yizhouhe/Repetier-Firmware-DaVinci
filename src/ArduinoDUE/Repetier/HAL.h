...
// You can set different sizes if you want, but with binary mode it does not get
// faster
...
//#define	READ(pin)  PIO_Get(g_APinDescription[pin].pPort, PIO_INPUT,
// g_APinDescription[pin].ulPin)
#define READ_VAR(pin) \
    (g_APinDescription[pin].pPort->PIO_PDSR & g_APinDescription[pin].ulPin \
         ? 1 \
         : 0) // does return 0 or pin value
#define _READ(pin) \
    (DIO##pin##_PORT->PIO_PDSR & DIO##pin##_PIN \
         ? 1 \
         : 0) // does return 0 or pin value
#define READ(pin) _READ(pin)
//#define	WRITE_VAR(pin, v) PIO_SetOutput(g_APinDescription[pin].pPort,
// g_APinDescription[pin].ulPin, v, 0, PIO_PULLUP)
...
// Uses ABSEPRI to allow higher level interrupts then the one changing firmware
// data
#if 1
class InterruptProtectedBlock {
public:
    INLINE void protect() { __disable_irq(); }

    INLINE void unprotect() { __enable_irq(); }
    ...
    INLINE ~InterruptProtectedBlock() { __enable_irq(); }
};
#else
class InterruptProtectedBlock {
    uint32_t mask;
...
public:
    ...
    inline void unprotect() { __set_PRIMASK(mask); }
    ...
    inline ~InterruptProtectedBlock() { __set_PRIMASK(mask); }
};
#endif
...
class HAL {
public:
    // we use ram instead of eeprom, so reads are faster and safer. Writes store
    // in real eeprom as well as long as hal eeprom functions are used.
    ...
    static inline void hwSetup(void) {
        ...
        TC_Configure(DELAY_TIMER, DELAY_TIMER_CHANNEL,
                     TC_CMR_WAVSEL_UP | TC_CMR_WAVE | DELAY_TIMER_CLOCK);
    ...
    static inline unsigned int ComputeV(long timer, long accel) {
        return static_cast<unsigned int>(
            (static_cast<int64_t>(timer) * static_cast<int64_t>(accel)) >> 18);
    ...
    static inline uint8_t digitalRead(uint8_t pin) { return READ_VAR(pin); }
    ...
    static long CPUDivU2(speed_t divisor) { return F_CPU / divisor; }
    static INLINE void delayMicroseconds(uint32_t usec) { // usec += 3;
        uint32_t n = usec * (F_CPU_TRUE / 3000000);
        asm volatile("L2_%=_delayMicroseconds:"
                     "\n\t"
                     "subs   %0, #1"
                     "\n\t"
    ...
    static inline void tone(uint8_t pin, int frequency) {
        ...
        TC_Configure(BEEPER_TIMER, BEEPER_TIMER_CHANNEL,
                     TC_CMR_WAVE | TC_CMR_WAVSEL_UP_RC | TC_CMR_TCCLKS_TIMER_CLOCK4); // TIMER_CLOCK4 -> 128 divisor
        BEEPER_TIMER->TC_CHANNEL[BEEPER_TIMER_CHANNEL].TC_IER = TC_IER_CPCS;          // RC compare interrupt
        ...
        uint32_t rc = VARIANT_MCK / 256 / frequency;
    ...
    static inline void eprBurnValue(unsigned int pos, int size,
                                    union eeval_t newvalue) {
        ...
        WRITE(SPI_EEPROM1_CS, HIGH);
        delayMilliseconds(
            EEPROM_PAGE_WRITE_TIME); // wait for page write to complete
    ...
    spiSendBlock(uint8_t token, const uint8_t* buf) {
        // WRITE(SDSS, LOW);
        spiTransfer(token);
    ...
    // Due can only go as slow as AVR divider 32 -- slowest Due clock is 329,412
    // Hz
