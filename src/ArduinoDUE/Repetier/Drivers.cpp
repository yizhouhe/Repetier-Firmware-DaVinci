...
MotorDriverInterface* motorDrivers[NUM_MOTOR_DRIVERS] = { &motorDriver1
#if NUM_MOTOR_DRIVERS > 1
                                                          ,
                                                          &motorDriver2
#endif
...
MotorDriverInterface* getMotorDriver(int idx) { return motorDrivers[idx]; }
...
// E1 = home only if endstop was never met, meaning it was never homed with
// motor.
void commandG205(GCode& code) {
    ...
    if (id >= NUM_MOTOR_DRIVERS)
        id = 0;
    motorDrivers[id]->home(code.hasS() && code.S != 0,
                           code.hasE() && code.E != 0);
}
...
secondspeed_t LaserDriver::intensity = LASER_PWM_MAX; // Intensity to use for next move queued if we want lasers.
