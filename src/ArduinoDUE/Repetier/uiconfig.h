...
The ui adds quite some code, so AVRs with 64kB ram (Sanguino, Gen6) can not
handle all features of the firmware at the same time. You have to disable some
features to gain the ram needed. What should work:
...
==============================================================*/
  ...
  works, use the ascii charset 0 as fallback. Not the nicest for everything but
  working!
  ...
  */
  ...
  4 = Use the slower LiquiedCrystal library bundled with arduino.
      IMPORTANT: You need to uncomment the LiquidCrystal include in Repetier.pde
      for it to work. If you have Sanguino and want to use the library, you need to
      have Arduino 023 or older. (13.04.2012) 
  ...
  */
  ...
  // 0x40 till 0x4e for PCF8574, 0x40 for the adafruid RGB shield, 0x40 - 0x4e for
  // MCP23017 Official addresses have a value half as high!
  ...
  // Set the output mask that is or'd over the output data. This is needed to
  // activate a backlight switched over the I2C. The adafruit RGB shields enables
  // a light if the bit is not set. Bits 6-8 are used for backlight.
  #define UI_DISPLAY_I2C_OUTPUT_START_MASK 0
  // For MCP which inputs are with pullup. 31 = pins 0-4 for adafruid rgb shield
  // buttons
  #define UI_DISPLAY_I2C_PULLUP 31
  /* How fast should the I2C clock go. The PCF8574 work only with the lowest
  setting 100000. A MCP23017 can run also with 400000 Hz */
  ...
  */
  ...
  If you have menus enabled, you need a method to leave it. If you have a back
  key, you can always go one level higher. Without a back key, you need to
  navigate to the back entry in the menu. Setting this value to 1 removes the back
  entry.
  */
  #define UI_HAS_BACK_KEY 1
  
  /* Then you have the next/previous keys more like up/down keys, it may be more
  intuitive to change the direction you skip through the menus. If you set it to
  true, next will go to previous menu instead of the next menu.
  ...
  */
  ...
  // Under which address can the key status requested. This is the address of your
  // PCF8574 where the keys are connected. If you use a MCP23017 the address from
  // display is used also for keys.
  ...
  For each key, two seperate parts must be defined. The first is the
  initialization which must be added inside uiInitKeys() and the second ist a
  testing routine. These come into uiCheckKeys() or uiCheckSlowKeys() depending on
  the time needed for testing. If you are in doubt, put it in uiCheckSlowKeys().
  ...
  at the beginning of ui.h It's best to use the symbol name, in case the value
  changes.
  ...
  3. Click encoder, A/B connected to gnd if closed.
      ...
      If you can move the menu cursor without a click, just be adding some force
  in one direction, toggle the _REV with non _REV and toggle pins. If the
  direction is wrong, toggle _REV with non _REV version. For the push button of
  the encoder use 1.
  ...
  All keys and the buzzer if present must be on a connected to a single PCF8574
  chip! As all I2C request take time, they belong all in uiCheckSlowKeys. Dont use
  the pin ids but instead _BV(pinNumber0_7) as pin id. 0 = First pin
  ...
  in one direction, toggle the _REV with non _REV and toggle pins. If the
  direction is wrong, toggle _REV with non _REV version. For the push button of
  the encoder use 7. NOTICE: The polling frequency is limited, so only slow turns
  ...
  Type 1: Immediate actions - these are execute and forget actions like
  home/pre-heat Type 2: Parameter change action - these change the mode for
  next/previous keys. They are valid until a new change action is initiated or the
  action is finished with ok button. Type 3: Show menu action. These actions have
  a _MENU_ in their name. If they are executed, a new menu is pushed on the menu
  stack and you see the menu. If you assign these actions directly to a key, you
  might not want this pushing behaviour. In this case add UI_ACTION_TOPMENU to the
          action, like UI_ACTION_TOPMENU+UI_ACTION_MENU_XPOSFAST. That will show
  the menu as top-menu closing all othe submenus that were open.
  
     ####################################################################### */
  
  // Use these codes for key detect. The main menu will show the pressed action in
  // the lcd display. after that assign the desired codes.
  ...
  void uiInitKeys() {
      ...
      //UI_KEYS_INIT_CLICKENCODER_LOW(47,45); // click encoder on pins 47 and 45.
      //Phase is connected with gnd for signals. UI_KEYS_INIT_BUTTON_LOW(43); //
  ...
  //UI_KEYS_BUTTON_LOW(42,
  //UI_ACTION_SD_PRINT); // push button, connects gnd to pin
  //UI_KEYS_CLICKENCODER_LOW_REV(47,45); // click encoder on pins 47 and 45.
  //Phase is connected with gnd for signals.
  ...
  #endif
      HAL::i2cStop();
      // Add I2C click encoder tests here, all other i2c tests and a copy of the
      // encoder test belog in uiCheckSlowKeys
      UI_KEYS_I2C_CLICKENCODER_LOW_REV(
          _BV(2), _BV(0)); // click encoder on pins 0 and 2. Phase is connected with
                           // gnd for signals.
  #endif
  }
  void uiCheckSlowKeys(uint16_t& action) {
      ...
      UI_KEYS_I2C_CLICKENCODER_LOW_REV(
          _BV(2), _BV(0)); // click encoder on pins 0 and 2. Phase is connected with
                           // gnd for signals.
      UI_KEYS_I2C_BUTTON_LOW(_BV(1),
                             UI_ACTION_OK); // push button, connects gnd to pin
      UI_KEYS_I2C_BUTTON_LOW(_BV(3),
                             UI_ACTION_BACK); // push button, connects gnd to pin
      UI_KEYS_I2C_BUTTON_LOW(
          _BV(4), UI_ACTION_MENU_QUICKSETTINGS + UI_ACTION_TOPMENU); // push button, connects gnd to pin
      UI_KEYS_I2C_BUTTON_LOW(
          _BV(5), UI_ACTION_MENU_EXTRUDER + UI_ACTION_TOPMENU); // push button, connects gnd to pin
      UI_KEYS_I2C_BUTTON_LOW(
          _BV(6), UI_ACTION_MENU_POSITIONS + UI_ACTION_TOPMENU); // push button, connects gnd to pin
  /*
