...
/** Allows M111 to set bit 5 (16) which disables all commands except M111. This
can be used to test your data throughput or search for communication problems.
*/
#define INCLUDE_DEBUG_COMMUNICATION 1
// Echo all ascii commands after receiving
//#define DEBUG_ECHO_ASCII
/** Allows M111 so set bit 6 (32) which disables moves, at the first tried step.
In combination with a dry run, you can test the speed of path computations,
which are still performed. */
#define INCLUDE_DEBUG_NO_MOVE 1
/** Writes the free RAM to output, if it is less then at the last test. Should
always return values >500 for safety, since it doesn't catch every function
call. Nice to tweak cache usage or for searching for memory induced errors.
Switch it off for production, it costs execution time. */
...
/** This enables code to make M666 drop an ok, so you get problems with
 * communication. It is to test host robustness. */
//#define DEBUG_COM_ERRORS
/** Adds a menu point in quick settings to write debug informations to the host
 * in case of hangs where the ui still works. */
//#define DEBUG_PRINT
...
// Uncomment the following line to enable debugging. You can better control
...
// we can not prevent this as some configurations need a parameter and others
// not
...
#endif
// After this count of steps a new SIN / COS calculation is started to correct
// the circle interpolation
...
extern uint8_t
    pwm_pos[NUM_PWM]; // 0-NUM_EXTRUDER = Heater 0-NUM_EXTRUDER of extruder,
                      // NUM_EXTRUDER = Heated bed, NUM_EXTRUDER+1 Board fan,
                      // NUM_EXTRUDER+2 = Fan
#if USE_ADVANCE
...
#if NONLINEAR_SYSTEM
extern uint8_t transformCartesianStepsToDeltaSteps(long cartesianPosSteps[],
                                                   long deltaPosSteps[]);
