...
void HAL::setupTimer() {
    ...
    // NVIC_SetPriority((IRQn_Type)EXTRUDER_TIMER_IRQ, NVIC_EncodePriority(4, 4,
    // 1));
    NVIC_SetPriority((IRQn_Type)EXTRUDER_TIMER_IRQ, 6);

    // count up to value in RC register using given clock
    TC_Configure(EXTRUDER_TIMER, EXTRUDER_TIMER_CHANNEL,
                 TC_CMR_WAVSEL_UP_RC | TC_CMR_WAVE | TC_CMR_TCCLKS_TIMER_CLOCK3);

    TC_SetRC(EXTRUDER_TIMER, EXTRUDER_TIMER_CHANNEL,
             (F_CPU_TRUE / 32) / EXTRUDER_CLOCK_FREQ); // set frequency 43 for 60000Hz
    ...
    TC_FindMckDivisor(PWM_CLOCK_FREQ, F_CPU_TRUE, &tc_count, &tc_clock,
                      F_CPU_TRUE);
    TC_Configure(PWM_TIMER, PWM_TIMER_CHANNEL,
                 TC_CMR_WAVSEL_UP_RC | TC_CMR_WAVE | tc_clock);

    TC_SetRC(PWM_TIMER, PWM_TIMER_CHANNEL,
             (F_CPU_TRUE / tc_count) / PWM_CLOCK_FREQ);
    ...
    // NVIC_SetPriority((IRQn_Type)TIMER1_TIMER_IRQ, NVIC_EncodePriority(4, 7,
    // 1)); // highest priority - no surprises here wanted
    NVIC_SetPriority((IRQn_Type)TIMER1_TIMER_IRQ,
                     2); // highest priority - no surprises here wanted

    TC_Configure(TIMER1_TIMER, TIMER1_TIMER_CHANNEL,
                 TC_CMR_WAVSEL_UP_RC | TC_CMR_WAVE | TC_CMR_TCCLKS_TIMER_CLOCK1);

    TC_SetRC(TIMER1_TIMER, TIMER1_TIMER_CHANNEL,
             (F_CPU_TRUE / TIMER1_PRESCALE) / TIMER1_CLOCK_FREQ);
    ...
    TC_Configure(SERVO_TIMER, SERVO_TIMER_CHANNEL,
                 TC_CMR_WAVSEL_UP_RC | TC_CMR_WAVE | TC_CMR_TCCLKS_TIMER_CLOCK1);

    TC_SetRC(SERVO_TIMER, SERVO_TIMER_CHANNEL,
             (F_CPU_TRUE / SERVO_PRESCALE) / SERVO_CLOCK_FREQ);
...
static PWMChannel pwm_channel[8] = { { false, nullptr, 0 }, { false, nullptr, 0 }, { false, nullptr, 0 }, { false, nullptr, 0 }, { false, nullptr, 0 }, { false, nullptr, 0 }, { false, nullptr, 0 }, { false, nullptr, 0 } };

static void computePWMDivider(uint32_t frequency, uint32_t& div,
                              uint32_t& scale) {
    uint32_t factor = 1;
    div = 0;
    do {
...
int HAL::initHardwarePWM(int pinNumber, uint32_t frequency) {
    ...
    PIO_Configure(p.pio,
                  ...
                  p.pio_pin, PIO_DEFAULT);

    PWMC_ConfigureChannelExt(PWM_INTERFACE,
                             p.channel,               // channel
                             div,                     // clock divider
                             0,                       // left aligned
...
void HAL::analogStart(void) {
    ...
    PIO_Configure(g_APinDescription[58].pPort, g_APinDescription[58].ulPinType,
                  g_APinDescription[58].ulPin,
                  g_APinDescription[58].ulPinConfiguration);
    PIO_Configure(g_APinDescription[59].pPort, g_APinDescription[59].ulPinType,
    ...
    // Initialize ADC mode register (some of the following params are not used
    // here) HW trigger disabled, use external Trigger, 12 bit resolution core and
    // ref voltage stays on, normal sleep mode, normal not free-run mode startup
    // time 16 clocks, settling time 17 clocks, no changes on channel switch
    ...
    if (eprSyncTime && (time - eprSyncTime > 15000)) { // Buffer writes only every 15 seconds to pool writes
        ...
        if (!sd.sdactive) { // not mounted
            if (eepromFile.isOpen())
                eepromFile.close();
            Com::printErrorF(
                "Could not write eeprom to sd card - no sd card mounted");
            Com::println();
            return;
        }
...
uint32_t HAL::integer64Sqrt(uint64_t a_nInput) {
    ...
    uint64_t res = 0;
    uint64_t one = 1uLL << 62; // The second-to-top bit is set: use 1u << 14 for
...
void HAL::spiBegin(uint8_t ssPin) {
    ...
    if (spiInitMaded == false) {
        ...
        PIO_Configure(g_APinDescription[MISO_PIN].pPort,
...
#endif // MOTHERBOARD == 500 || MOTHERBOARD == 501 || (MOTHERBOARD==502)
        PIO_Configure(g_APinDescription[SPI_PIN].pPort,
                      g_APinDescription[SPI_PIN].ulPinType,
                      g_APinDescription[SPI_PIN].ulPin,
                      g_APinDescription[SPI_PIN].ulPinConfiguration);
        HAL::digitalWrite(ssPin, 1);
        spiInit(1);
#if (MOTHERBOARD == 500) || (MOTHERBOARD == 501) || (MOTHERBOARD == 502)
        spiInitMaded = true;
    }
...
void HAL::spiInit(uint8_t spiClock) {
    ...
    if (spiInitMaded == false) {
        ...
        if (spiClock > 4)
            spiClock = 1;
#if MOTHERBOARD == 500 || MOTHERBOARD == 501 || (MOTHERBOARD == 502)
        // Set SPI mode 1, clock, select not active after transfer, with delay
        // between transfers
        SPI_ConfigureNPCS(SPI0, SPI_CHAN_DAC,
                          SPI_CSR_CSAAT | SPI_CSR_SCBR(spiDueDividors[spiClock]) | SPI_CSR_DLYBCT(1));
        // Set SPI mode 0, clock, select not active after transfer, with delay
        // between transfers
        SPI_ConfigureNPCS(SPI0, SPI_CHAN_EEPROM1,
                          SPI_CSR_NCPHA | SPI_CSR_CSAAT | SPI_CSR_SCBR(spiDueDividors[spiClock]) | SPI_CSR_DLYBCT(1));
#endif // MOTHERBOARD==500 || MOTHERBOARD==501 || (MOTHERBOARD==502)

        // Set SPI mode 0, clock, select not active after transfer, with delay
        // between transfers
        SPI_ConfigureNPCS(SPI0, BOARD_PIN_TO_SPI_CHANNEL(SPI_PIN),
                          SPI_CSR_NCPHA | SPI_CSR_CSAAT | SPI_CSR_SCBR(spiDueDividors[spiClock]) | SPI_CSR_DLYBCT(1));
        SPI_Enable(SPI0);
#if MOTHERBOARD == 500 || MOTHERBOARD == 501 || (MOTHERBOARD == 502)
        spiInitMaded = true;
...
void HAL::spiSend(byte b) {
    ...
    SPI0->SPI_TDR = (uint32_t)b | SPI_TDR_LASTXFER | SPI_PCS(BOARD_PIN_TO_SPI_CHANNEL(SPI_PIN));
...
void HAL::spiSend(const uint8_t* buf, size_t n) {
    ...
    for (size_t i = 0; i < n - 1; i++) {
        SPI0->SPI_TDR = (uint32_t)buf[i] | SPI_PCS(BOARD_PIN_TO_SPI_CHANNEL(SPI_PIN));
        while ((SPI0->SPI_SR & SPI_SR_TDRE) == 0)
            ;
        while ((SPI0->SPI_SR & SPI_SR_RDRF) == 0)
            ;
...
uint8_t HAL::spiReceive() {
    ...
    SPI0->SPI_TDR = 0x000000FF | SPI_TDR_LASTXFER | SPI_PCS(BOARD_PIN_TO_SPI_CHANNEL(SPI_PIN));
    // wait for transmit register empty
    while ((SPI0->SPI_SR & SPI_SR_TDRE) == 0)
        ;
...
void HAL::spiReadBlock(uint8_t* buf, uint16_t nbyte) {
    ...
    for (int i = 0; i < nbyte; i++) {
        // while ((SPI0->SPI_SR & SPI_SR_TDRE) == 0);
        SPI0->SPI_TDR = 0x000000FF | SPI_PCS(BOARD_PIN_TO_SPI_CHANNEL(SPI_PIN));
        while ((SPI0->SPI_SR & SPI_SR_RDRF) == 0)
            ;
        buf[i] = SPI0->SPI_RDR;
        // delayMicroseconds(1);
...
void HAL::spiSendBlock(uint8_t token, const uint8_t* buf) {
    SPI0->SPI_TDR = (uint32_t)token | SPI_PCS(BOARD_PIN_TO_SPI_CHANNEL(SPI_PIN));
    while ((SPI0->SPI_SR & SPI_SR_TDRE) == 0)
        ;
    // while ((SPI0->SPI_SR & SPI_SR_RDRF) == 0);
    // SPI0->SPI_RDR;
...
#define STEPPERTIMER_EXIT_TICKS \
    105 // at least 2,5us pause between stepper calls
#endif
...
void TIMER1_COMPA_VECTOR() {
    ...
    InterruptProtectedBlock
        noInt; // prevent interruption or we might get 102s delay
    if (stepperChannel->TC_CV + STEPPERTIMER_EXIT_TICKS > timer_count) {
        stepperChannel->TC_RC = stepperChannel->TC_CV + STEPPERTIMER_EXIT_TICKS; // should end after exiting timer interrupt
                                                                                 // stepperChannel->TC_CCR = TC_CCR_CLKEN |
                                                                                 // TC_CCR_SWTRG ;
    } else {
...
void PWM_TIMER_VECTOR() {
    ...
    if (pwm_count_heater == 0 && !PDM_FOR_EXTRUDER) {
        ...
        if ((pwm_pos_set[NUM_EXTRUDER] = pwm_pos[NUM_EXTRUDER]) > 0)
...
#if defined(EXT0_HEATER_PIN) && EXT0_HEATER_PIN > -1
#if PDM_FOR_EXTRUDER
    pulseDensityModulate(EXT0_HEATER_PIN, pwm_pos[0], pwm_pos_set[0],
...
#if PDM_FOR_COOLER
    pulseDensityModulate(EXT0_EXTRUDER_COOLER_PIN, extruder[0].coolerPWM,
                         pwm_cooler_pos_set[0], false);
#else
...
#if PDM_FOR_COOLER
    pulseDensityModulate(EXT1_EXTRUDER_COOLER_PIN, extruder[1].coolerPWM,
                         pwm_cooler_pos_set[1], false);
#else
...
#if defined(EXT2_HEATER_PIN) && EXT2_HEATER_PIN > -1 && NUM_EXTRUDER > 2 && !MIXING_EXTRUDER
#if PDM_FOR_EXTRUDER
    pulseDensityModulate(EXT2_HEATER_PIN, pwm_pos[2], pwm_pos_set[2],
                         HEATER_PINS_INVERTED);
#else
...
#if PDM_FOR_COOLER
    pulseDensityModulate(EXT2_EXTRUDER_COOLER_PIN, extruder[2].coolerPWM,
                         pwm_cooler_pos_set[2], false);
#else
...
#if defined(EXT3_HEATER_PIN) && EXT3_HEATER_PIN > -1 && NUM_EXTRUDER > 3 && !MIXING_EXTRUDER
#if PDM_FOR_EXTRUDER
    pulseDensityModulate(EXT3_HEATER_PIN, pwm_pos[3], pwm_pos_set[3],
                         HEATER_PINS_INVERTED);
#else
...
#if PDM_FOR_COOLER
    pulseDensityModulate(EXT3_EXTRUDER_COOLER_PIN, extruder[3].coolerPWM,
                         pwm_cooler_pos_set[3], false);
#else
...
#if defined(EXT4_HEATER_PIN) && EXT4_HEATER_PIN > -1 && NUM_EXTRUDER > 4 && !MIXING_EXTRUDER
#if PDM_FOR_EXTRUDER
    pulseDensityModulate(EXT4_HEATER_PIN, pwm_pos[4], pwm_pos_set[4],
                         HEATER_PINS_INVERTED);
#else
...
#if PDM_FOR_COOLER
    pulseDensityModulate(EXT4_EXTRUDER_COOLER_PIN, extruder[4].coolerPWM,
                         pwm_cooler_pos_set[4], false);
#else
...
#if defined(EXT5_HEATER_PIN) && EXT5_HEATER_PIN > -1 && NUM_EXTRUDER > 5 && !MIXING_EXTRUDER
#if PDM_FOR_EXTRUDER
    pulseDensityModulate(EXT5_HEATER_PIN, pwm_pos[5], pwm_pos_set[5],
                         HEATER_PINS_INVERTED);
#else
...
#if PDM_FOR_COOLER
    pulseDensityModulate(EXT5_EXTRUDER_COOLER_PIN, extruder[5].coolerPWM,
                         pwm_cooler_pos_set[5], false);
#else
...
#if FAN_BOARD_PIN > -1 && SHARED_COOLER_BOARD_EXT == 0
#if PDM_FOR_COOLER
    pulseDensityModulate(FAN_BOARD_PIN, pwm_pos[PWM_BOARD_FAN],
                         pwm_pos_set[PWM_BOARD_FAN], false);
#else
    ...
    if (fanKickstart == 0) {
#if PDM_FOR_COOLER
        pulseDensityModulate(FAN_PIN, pwm_pos[PWM_FAN1], pwm_pos_set[PWM_FAN1],
                             false);
#else
    ...
    if (fan2Kickstart == 0) {
#if PDM_FOR_COOLER
        pulseDensityModulate(FAN2_PIN, pwm_pos[PWM_FAN2], pwm_pos_set[PWM_FAN2],
                             false);
#else
...
#if defined(FAN_THERMO_PIN) && FAN_THERMO_PIN > -1
#if PDM_FOR_COOLER
    pulseDensityModulate(FAN_THERMO_PIN, pwm_pos[PWM_FAN_THERMO],
                         pwm_pos_set[PWM_FAN_THERMO], false);
#else
...
#if HEATED_BED_HEATER_PIN > -1 && HAVE_HEATED_BED
#if PDM_FOR_EXTRUDER
    pulseDensityModulate(HEATED_BED_HEATER_PIN, pwm_pos[NUM_EXTRUDER],
                         pwm_pos_set[NUM_EXTRUDER], HEATER_PINS_INVERTED);
#else
...
#define SLOW_EXTRUDER_TICKS \
    (F_CPU_TRUE / 32 / 1000) // 250us on direction change
#define NORMAL_EXTRUDER_TICKS \
    (F_CPU_TRUE / 32 / EXTRUDER_CLOCK_FREQ) // 500us on direction change
#ifndef ADVANCE_DIR_FILTER_STEPS
#define ADVANCE_DIR_FILTER_STEPS 2
#endif

static int extruderLastDirection = 0;
void HAL::resetExtruderDirection() { extruderLastDirection = 0; }
