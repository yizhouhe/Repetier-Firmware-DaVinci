...
void SDCard::unmount() {
    ...
    Printer::setAutomount(false);
    Printer::setMenuMode(
        MENU_MODE_SD_MOUNTED + MENU_MODE_PAUSED + MENU_MODE_SD_PRINTING, false);
...
void SDCard::JSONFileInfo(const char* filename) {
    ...
    // {"err":0,"size":457574,"height":4.00,"layerHeight":0.25,"filament":[6556.3],"generatedBy":"Slic3r
    // 1.1.7 on 2014-11-09 at 17:11:32"}
...
bool GCodeFileInfo::findFilamentNeed(char* buf, float& filament) {
    ...
    if (pos != NULL) {
        ...
        while (*pos == ' ' || *pos == 't' || *pos == '=' || *pos == ':') {
            ++pos; // this allows for " = " from default slic3r comment and ": " from
                   // default Cura comment
        }
        if (isDigit(*pos)) {
            char* q;
...
bool GCodeFileInfo::findTotalHeight(char* buf, float& height) {
    ...
    for (int i = len - 5; i > 0; i--) {
        if (inRelativeMode) {
            ...
            // Ignore G0/G1 codes if absolute mode was switched back using G90
            // (typical for Cura files)
            if (buf[i + 1] == '9' && buf[i + 2] == '0' && buf[i + 3] <= ' ') {
                inRelativeMode = true;
            } else if ((buf[i + 1] == '0' || buf[i + 1] == '1') && buf[i + 2] == ' ') {
                // Look for last "G0/G1 ... Z#HEIGHT#" command as generated by common
                // slicers Looks like we found a controlled move, however it could be in
                // a comment, especially when using slic3r 1.1.1
                inComment = false;
                size_t j = i;
                while (j != 0) {
                    --j;
