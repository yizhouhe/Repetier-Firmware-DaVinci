...
#endif
#ifdef SUPPORT_MAX31855_SW
extern int16_t read_max31855_sw_spi(uint8_t inPin, fast8_t idx);
#endif
    ...
    for (uint8_t controller = 0; controller < NUM_TEMPERATURE_LOOPS; controller++) {
        ...
        if (controller < NUM_EXTRUDER) {
            ...
            if (controller == 0) {
                ...
                if (pwm_pos[PWM_BOARD_FAN] == BOARD_FAN_SPEED)
                    enable = true;
#endif
        ...
        if (controller == autotuneIndex)
            continue;
#if MIXING_EXTRUDER
        if (controller > 0 && controller < NUM_EXTRUDER)
            continue; // Mixing extruder only test for ext 0
#endif                // MIXING_EXTRUDER
        ...
        if ((act->currentTemperatureC < MIN_DEFECT_TEMPERATURE || act->currentTemperatureC > MAX_DEFECT_TEMPERATURE) && act->targetTemperatureC > 0 /*is heating*/ && (act->preheatTime() == 0 || act->preheatTime() >= MILLISECONDS_PREHEAT_TIME /*preheating time is over*/)) { // no temp sensor or short in sensor, disable heater
            errorDetected = 1;
...
#endif
#if defined(SUPPORT_MAX31855_SW)
    if (heatedBedController.sensorType == 103) {
        SET_OUTPUT(MAX31855_SW_CLK);
        WRITE(MAX31855_SW_CLK, 0);
        SET_OUTPUT(MAX31855_SW_CS);
        WRITE(MAX31855_SW_CS, 1);
        HAL::pinMode(heatedBedController.sensorPin, INPUT);
    }
#endif
#endif
}
...
#endif
#if defined(SUPPORT_MAX31855_SW)
        if (act->tempControl.sensorType == 103) {
            SET_OUTPUT(MAX31855_SW_CLK);
            WRITE(MAX31855_SW_CLK, 0);
            SET_OUTPUT(MAX31855_SW_CS);
            WRITE(MAX31855_SW_CS, 1);
...
void Extruder::selectExtruderById(uint8_t extruderId) {
    ...
    if (Printer::maxExtruderSpeed > 40)
        Printer::maxExtruderSpeed = 40;
#else
    if (Printer::maxExtruderSpeed > 15)
...
void Extruder::setTemperatureForExtruder(float temperatureInCelsius, uint8_t extr, bool beep, bool wait) {
    ...
    if (Extruder::dittoMode && extr == 0) {
        ...
        if (temperatureInCelsius >= EXTRUDER_FAN_COOL_TEMP)
            extruder[1].coolerPWM = extruder[1].coolerSpeed;
#if NUM_EXTRUDER > 2
        if (Extruder::dittoMode > 1 && extr == 0) {
            ...
            tc2->updateTempControlVars();
            if (temperatureInCelsius >= EXTRUDER_FAN_COOL_TEMP)
                extruder[2].coolerPWM = extruder[2].coolerSpeed;
        }
#endif
#if NUM_EXTRUDER > 3
        if (Extruder::dittoMode > 2 && extr == 0) {
            ...
            tc2->updateTempControlVars();
            if (temperatureInCelsius >= EXTRUDER_FAN_COOL_TEMP)
                extruder[3].coolerPWM = extruder[3].coolerSpeed;
        }
#endif
    }
...
void Extruder::setHeatedBedTemperature(float temperatureInCelsius, bool beep) {
    ...
    if (beep && temperatureInCelsius > 30)
        heatedBedController.setAlarm(true);
...
const short temptable_13[NUMTEMPS_13][2] PROGMEM = {
    { 0, 0 }, { 184, 10 * 8 }, { 361, 20 * 8 }, { 531, 30 * 8 }, { 1002, 8 * 60 }, { 1554, 8 * 100 }, { 2034, 8 * 140 }, { 2455, 8 * 180 }, { 2647, 8 * 200 }, { 2827, 8 * 220 }, { 2998, 8 * 240 }, { 3159, 8 * 260 }, { 3312, 8 * 280 }, { 3456, 8 * 300 }, { 3593, 8 * 320 }, { 3724, 8 * 340 }, { 3848, 8 * 360 }, { 3966, 8 * 380 }, { 4079, 8 * 400 }, { 4095, 8 * 1000 }
};
#else
...
#endif
#ifdef SUPPORT_MAX31855_SW
    case 103: { // MAX31855 with software SPI
        int16_t newTemp = read_max31855_sw_spi(sensorPin, pwmIndex);
        if (newTemp != 20000) { // don't use error read
            currentTemperature = newTemp;
        }
    } break;
...
void TemperatureController::setTargetTemperature(float target) {
    ENSURE_POWER
    if (sensorType == 0) { // fake sensors can not heat up!
        target = 0;
    }
...
int16_t read_max31855_sw_spi(uint8_t inPin, fast8_t idx) {
    ...
    for (fast8_t i = 0; i < 32; i++) { // read 32 bit
        //  SCK high
        WRITE(MAX31855_SW_CLK, 1);
        HAL::delayMicroseconds(1);
        data += data + (HAL::digitalRead(inPin) ? 1 : 0);
        WRITE(MAX31855_SW_CLK, 0);
        HAL::delayMicroseconds(1);
    }
    WRITE(MAX31855_SW_CS, 1); // stop reading

    // Com::printF(PSTR("Max:"), (int32_t)(data >> 16));
    // Com::printFLN(PSTR(" / "),(int32_t)(data & 65535));
    //Process temp
    if (data & 65536 /* 0x00010000 */) { // test error flag
        if (max31855_errors[idx] > 5)
            return -396; // will trigger defect when heating, -99°C so it fits into display
...
/*
void readAll8Thermocouples( void )
{
	//  CS low
	REG_PIOB_CODR = 1;
	delayMicroseconds( 1 );
	//  read 2 bits from all channels simultaneously
	uint32_t buf[ 8 ];
	for( int bits = 0; bits < 32; ++bits )
	{
		...
		REG_PIOB_CODR = 2;
		delayMicroseconds( 1 );
		//  read a local copy of all bits
		uint32_t allbits = REG_PIOB_PDSR;
		allbits >>= 2;  //  skip the CS and SCK bits
		for( int b = 0; b < 8; ++b )
		{
			buf[ b ] += buf[ b ] + (allbits & 1);
			allbits >>= 1;
		}
	}
	//  CS high
...
#if NUM_EXTRUDER > 0
    { 0, EXT0_X_OFFSET, EXT0_Y_OFFSET, EXT0_Z_OFFSET, EXT0_STEPS_PER_MM, EXT0_ENABLE_PIN, EXT0_ENABLE_ON, EXT0_MAX_FEEDRATE, EXT0_MAX_ACCELERATION, EXT0_MAX_START_FEEDRATE, 0, EXT0_WATCHPERIOD, EXT0_WAIT_RETRACT_TEMP, EXT0_WAIT_RETRACT_UNITS
#if USE_ADVANCE
...
#if NUM_EXTRUDER > 1
    ,
    { 1, EXT1_X_OFFSET, EXT1_Y_OFFSET, EXT1_Z_OFFSET, EXT1_STEPS_PER_MM, EXT1_ENABLE_PIN, EXT1_ENABLE_ON, EXT1_MAX_FEEDRATE, EXT1_MAX_ACCELERATION, EXT1_MAX_START_FEEDRATE, 0, EXT1_WATCHPERIOD, EXT1_WAIT_RETRACT_TEMP, EXT1_WAIT_RETRACT_UNITS
#if USE_ADVANCE
#if ENABLE_QUADRATIC_ADVANCE
      ,
      EXT1_ADVANCE_K
...
#endif
      ,
      { 2, EXT2_TEMPSENSOR_TYPE, EXT2_SENSOR_INDEX, EXT2_HEAT_MANAGER, 0, 0, 0, 0, 0, 0, 0, EXT2_PID_INTEGRAL_DRIVE_MAX, EXT2_PID_INTEGRAL_DRIVE_MIN, EXT2_PID_PGAIN_OR_DEAD_TIME, EXT2_PID_I, EXT2_PID_D, EXT2_PID_MAX, 0, 0, 0, 0, 0, EXT2_DECOUPLE_TEST_PERIOD, 0, EXT2_PREHEAT_TEMP
#if ENABLED(TEMP_GAIN)
        ,
        1.0, 0.0
#endif
...
#if NUM_EXTRUDER > 3
    ,
    { 3, EXT3_X_OFFSET, EXT3_Y_OFFSET, EXT3_Z_OFFSET, EXT3_STEPS_PER_MM, EXT3_ENABLE_PIN, EXT3_ENABLE_ON, EXT3_MAX_FEEDRATE, EXT3_MAX_ACCELERATION, EXT3_MAX_START_FEEDRATE, 0, EXT3_WATCHPERIOD, EXT3_WAIT_RETRACT_TEMP, EXT3_WAIT_RETRACT_UNITS
#if USE_ADVANCE
#if ENABLE_QUADRATIC_ADVANCE
      ,
      EXT3_ADVANCE_K
...
#endif
      ,
      { 3, EXT3_TEMPSENSOR_TYPE, EXT3_SENSOR_INDEX, EXT3_HEAT_MANAGER, 0, 0, 0, 0, 0, 0, 0, EXT3_PID_INTEGRAL_DRIVE_MAX, EXT3_PID_INTEGRAL_DRIVE_MIN, EXT3_PID_PGAIN_OR_DEAD_TIME, EXT3_PID_I, EXT3_PID_D, EXT3_PID_MAX, 0, 0, 0, 0, 0, EXT3_DECOUPLE_TEST_PERIOD, 0, EXT3_PREHEAT_TEMP
#if ENABLED(TEMP_GAIN)
        ,
        1.0, 0.0
#endif
...
#if NUM_EXTRUDER > 4
    ,
    { 4, EXT4_X_OFFSET, EXT4_Y_OFFSET, EXT4_Z_OFFSET, EXT4_STEPS_PER_MM, EXT4_ENABLE_PIN, EXT4_ENABLE_ON, EXT4_MAX_FEEDRATE, EXT4_MAX_ACCELERATION, EXT4_MAX_START_FEEDRATE, 0, EXT4_WATCHPERIOD, EXT4_WAIT_RETRACT_TEMP, EXT4_WAIT_RETRACT_UNITS
#if USE_ADVANCE
#if ENABLE_QUADRATIC_ADVANCE
      ,
      EXT4_ADVANCE_K
...
#endif
      ,
      { 4, EXT4_TEMPSENSOR_TYPE, EXT4_SENSOR_INDEX, EXT4_HEAT_MANAGER, 0, 0, 0, 0, 0, 0, 0, EXT4_PID_INTEGRAL_DRIVE_MAX, EXT4_PID_INTEGRAL_DRIVE_MIN, EXT4_PID_PGAIN_OR_DEAD_TIME, EXT4_PID_I, EXT4_PID_D, EXT4_PID_MAX, 0, 0, 0, 0, 0, EXT4_DECOUPLE_TEST_PERIOD, 0, EXT4_PREHEAT_TEMP
#if ENABLED(TEMP_GAIN)
        ,
        1.0, 0.0
#endif
...
#if NUM_EXTRUDER > 5
    ,
    { 5, EXT5_X_OFFSET, EXT5_Y_OFFSET, EXT5_Z_OFFSET, EXT5_STEPS_PER_MM, EXT5_ENABLE_PIN, EXT5_ENABLE_ON, EXT5_MAX_FEEDRATE, EXT5_MAX_ACCELERATION, EXT5_MAX_START_FEEDRATE, 0, EXT5_WATCHPERIOD, EXT5_WAIT_RETRACT_TEMP, EXT5_WAIT_RETRACT_UNITS
#if USE_ADVANCE
#if ENABLE_QUADRATIC_ADVANCE
      ,
      EXT5_ADVANCE_K
...
#endif
      ,
      { 5, EXT5_TEMPSENSOR_TYPE, EXT5_SENSOR_INDEX, EXT5_HEAT_MANAGER, 0, 0, 0, 0, 0, 0, 0, EXT5_PID_INTEGRAL_DRIVE_MAX, EXT5_PID_INTEGRAL_DRIVE_MIN, EXT5_PID_PGAIN_OR_DEAD_TIME, EXT5_PID_I, EXT5_PID_D, EXT5_PID_MAX, 0, 0, 0, 0, 0, EXT5_DECOUPLE_TEST_PERIOD, 0, EXT5_PREHEAT_TEMP
#if ENABLED(TEMP_GAIN)
        ,
        1.0, 0.0
#endif
...
TemperatureController thermoController = {
    PWM_FAN_THERMO, FAN_THERMO_THERMISTOR_TYPE, THERMO_ANALOG_INDEX, 0, 0, 0, 0, 0, 0, 0, 0, 255, 0, 10, 1, 1, 255, 0, 0, 0, 0, 0, 0, 0, 0
};
#endif
